...
const hsUrl = "http://localhost/curd";
const httpOptions = {
...
addHelps (help): Observable<Help> {
  ...
  return this.http.post<Help>(`${hsUrl}/create.php`, help, httpOptions).pipe(
...
updateHelps (id, help): Observable<any> {
  const url = `${hsUrl}/update.php?id=${id}`;
...
deleteHelps (id): Observable<Help> {
  const url = `${hsUrl}/delete.php?id=${id}`;
